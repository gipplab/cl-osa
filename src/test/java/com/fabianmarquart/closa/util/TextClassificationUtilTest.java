...
public class TextClassificationUtilTest {
    ...
    public void testClassify() {
        ...
        Assert.assertTrue(TextClassificationUtil.classifyText(englishText, "en").equals(TextClassificationUtil.Category.biology));
        Assert.assertTrue(TextClassificationUtil.classifyText(germanText, "de").equals(TextClassificationUtil.Category.biology));
        System.out.println(TextClassificationUtil.classifyText(japaneseText, "en"));
        System.out.println(TextClassificationUtil.classifyText(chineseText, "en"));
        System.out.println(TextClassificationUtil.classifyText(spanishText, "en"));
        System.out.println(TextClassificationUtil.classifyText(frenchText, "en"));
    }
    ...
    public void evaluateClassifier() {
        /*
            ...
            Classification for language es:
            Correct rate = NaN
            Incorrect rate = NaN
            Sample size = 0.0
         */
        ...
        for (String language : languages) {
            ...
            pathContentMap.entrySet()
                    .stream()
                    .filter(entry -> language.equals(TokenUtil.detectLanguage(entry.getValue())))
                    .forEach(entry -> {
                        File topicFile;
                        ...
                        TextClassificationUtil.Topic topic1 = null;

                        if (Files.exists(topicFile.toPath())) {
                            try {
                                topic1 = TextClassificationUtil.Topic.valueOf(FileUtils.readFileToString(topicFile, StandardCharsets.UTF_8));
                            } catch (IOException e) {
                                e.printStackTrace();
                            }
                        } else {
                            topic1 = TextClassificationUtil.uClassifyText(entry.getValue(), language);
                            try {
                        ...
                        TextClassificationUtil.Category topic2 = TextClassificationUtil.classifyText(entry.getValue(), language);

                        if (topic1.equals(TextClassificationUtil.Topic.Health)) {
                            if (topic2.equals(TextClassificationUtil.Category.biology)) {
                        ...
                        } else if ((topic1.equals(TextClassificationUtil.Topic.Arts) || topic1.equals(TextClassificationUtil.Topic.Games))) {
                            if (topic2.equals(TextClassificationUtil.Category.fiction)) {
                                correct.getAndIncrement();
                            } else {
                                incorrect.getAndIncrement();
                                // System.out.printf("Incorrect: Classified %s but should be %s%n", topic2, topic1);
    ...
    public void getClassificationCorpusFromWikipedia() {
        ...
        for (String language : languages) {
            ...
            TextClassificationUtil.getClassifierMap().put(language, bayesClassifier);
        }
    ...
    public void testUClassifyText() {
        String text = "2.1.4 virulence factors virulence factors are defined as bacterial products that cause direct " +
                "damage to the host cells. Virulence-associated factors enable the growth or Survival of the pathogen " +
                "in the host and in this way contribute to infection and illness (MAHAN et al. 1996";
        TextClassificationUtil.Topic topic = TextClassificationUtil.uClassifyText(text, "en");
        System.out.println(topic);

        Assert.assertTrue(topic.equals(TextClassificationUtil.Topic.Health));
        ...
        topic = TextClassificationUtil.uClassifyText(text, "en");
        System.out.println(topic);
    }
}
