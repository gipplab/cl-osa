...
def micro_avg_recall_and_precision(cases, detections):
    ...
    if num_plagiarized > 0:
        print "OWN Detected plagiarism: "+str(num_plagiarized_detected)
        print "OWN Overall plagiarism: "+str(num_plagiarized)
...
def case_recall(case, detections):
    ...
    """
    print "OWN Case Detected Plagarisim: "+str(num_detected_plagiarized)
    print "OWN Case Overall  Plagarisim: "+str(num_plagiarized)
    """
...
def extract_annotations_from_files(path, tagname, filter_mode, filter_on):
    ...
    for xmlfile in xmlfiles:
        annotations.update(extract_annotations_from_file(xmlfile, tagname, filter_mode, filter_on))
    return annotations


def extract_annotations_from_file(xmlfile, tagname,filter_mode, filter_on):
    ...
    for node in doc.documentElement.childNodes:
        if node.nodeType == xml.dom.Node.ELEMENT_NODE and \
            ...
            ann = extract_annotation_from_node(node, t_ref, filter_mode, filter_on)
            if ann:
                annotations.add(ann)
    return annotations
...
def extract_annotation_from_node(xmlnode, t_ref, filter_mode, filter_on):
    """Returns a plagiarism annotation from an XML feature tag node. ADDED OWNFILTER"""
    # filter_annotations = 'onlyManualTranslation'
    filter_annotations = filter_mode
...
def parse_options():
    ...
    if det_path == "undefined":
        ...
        sys.exit()
    return (micro_averaged, plag_path, plag_tag_name, det_path, det_tag_name, filter_mode)


def main(micro_averaged, plag_path, plag_tag_name, det_path, det_tag_name, filter_mode):
